...
def main():
    ...
    while not finished:
        removable_rolls_coords = [] #list of tuples (row,col)
        ...
        for r in range(num_rows):
            for c in range(num_cols):
                ...
                if neighbours.count(1) < 4:
                    count += 1
                    removable_rolls_coords.append((r, c))

                else:
                    continue

        # Remove the rolls
        if not removable_rolls_coords:
            finished = True
        else:
            if TEST:
                print("Cycle {}: Removing {} rolls".format(cycle, len(removable_rolls_coords)))
            for coord in removable_rolls_coords:
                row, col = coord
                map.cells[row][col] = 0
