...
def main():
    ...
    current_unused = AVAILABLE - root.this_dir_total_size
    diff_unused_and_needed = UPDATE_SIZE - current_unused 
    possible_dirs = []

    for i, dir in enumerate(FULL_LIST):

        if dir.this_dir_total_size >= diff_unused_and_needed:
    ...
    for i, dir in enumerate(possible_dirs):
        delete_sizes.append(dir.this_dir_total_size)

    size_of_dir_to_delete = min(delete_sizes)
    print(size_of_dir_to_delete)

    for i, dir in enumerate(FULL_LIST):
        if dir.this_dir_total_size == size_of_dir_to_delete:
            print(dir.name)
        else:
            continue
