...
class Symbol:
    ...
    def calculate_gear_ratio(self):
        total = 1
        for num in self.connected_parts:
            total *= int(num)

        self.gear_ratio = total
    ...
    for symbol in schematic.symbols:
        ...
        for number in schematic.numbers:
            ...
            if ((num_y == sym_y-1) and (sym_x_range.intersection(num_x))):
                number.is_part = True
                if symbol.name == "*":
                    symbol.connected_parts.append(number.num)
        ...
        if len(symbol.connected_parts) == 2:
            #print(symbol.calculate_gear_ratio())
            symbol.calculate_gear_ratio()
